	...
	MBox.prototype = {
		...
		getImages: function() {
			...
			if(tag == pover) {
				...
				var images = this.selector.find('.p_over').toArray(); // Find original images.
		...
		build: function() {
			...
			$(document).ready(function(){ 
				$("#mBox").appendTo(".body-overlay");
				$("body").css("overflow","hidden");
			});


			$(".mBox-current").append('<label class="mSide_l">' + this.currentImage.title + '</label>'); 
			var currentImageBox = $('<div/>', {
				'class': 'mBox-current'
			}).prependTo(container);

			console.log(this.currentImage.src);
			console.log(this.currentImage.title);
			...
			$('<label class="mSide_l">' + this.currentImage.title + '</label>').insertBefore('figure'); 
			// Image x of xx count.
			$('<p/>', {
		...
		createContainer: function() {
			...
			$("#mBox").append('<div class="mSide">'+'</div>'); 
			$(document).ready(function(){ 
        		$( ".side-bar-left" ).appendTo(".mSide");
    		});
