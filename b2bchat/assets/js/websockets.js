...
let connect = function () {
	...
	//ws.conn = new WebSocket('wss://' + socket_host + ':' + socket_port);
	ws.conn = new WebSocket("wss://volgopoint.com/wss/12");
	...
	ws.conn.onmessage = function (event) {
		...
		if (pkg.type === 'message') {
			...
			if ($('.user_uid').length < 1){
				...
				if(chat_user.db_id == pkg.to_user_db_id){
