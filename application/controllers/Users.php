...
class Users extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model('Json_Categories_Model');
	...
	public function ajax__get_form_by_sub_cat()
	{
		...
		$categories_std_arr = array();

		$jsonManipulation = (new \application\classes\JsonManipulation());
		$dynamic_forms = $jsonManipulation->get_dynamic_form();
        $single_form_data = $dynamic_forms->data;
		foreach ($single_form_data as $form){
			if($posted_data['sub_cat_id'] === $form->child_cat_id){
			array_push($categories_std_arr, $form->forms->form_category);
		    }
		}

		echo json_encode($categories_std_arr);
		exit;
	}
		...
		if (!empty($posted_data)) {
			...
			if (isset($_FILES['userfile']) && !empty($_FILES['userfile']['name'][0])) {
			    ...
			    for($i=0; $i<$cpt; $i++)
			    {
                    ...
                    $config = array();
                    $config['upload_path'] = $_SERVER['DOCUMENT_ROOT'].'/admin/uploads/listing_images/';
            ...
            foreach ($dataInfo as $key => $value) {
                unlink($_SERVER['DOCUMENT_ROOT'] . '/admin/uploads/listing_images/' . $value['file_name']);
            }
			...
			if ($this->form_validation->run() !== FALSE) {
				...
				if (isset($_FILES['cv_upload']) && !empty($_FILES['cv_upload']['name'])) {
					...
					$config['upload_path'] = $_SERVER['DOCUMENT_ROOT'].'/admin/uploads/cvs/';
					...
					if (!$this->upload->do_upload()) {
                        ...
                        unlink($_SERVER['DOCUMENT_ROOT'] . '/admin/uploads/cvs/' . $cv_data_info['file_name']);
					}
				}
    ...
    public function edit_ad_post($edit_id)
	{
		...
		if (!volgo_front_is_logged_in()) {
			redirect('login?redirected_to=' . base_url('edit-ad-post/') . $edit_id);
		}
		$getlistingdata = $this->Listings_Model->get_user_returned($edit_id);
        $images_already_available_arr = $this->Listings_Model->get_meta_returned_image($edit_id);
		if(isset($getlistingdata[0])){
            $listing_metas = json_decode($getlistingdata[0]->meta_values,true);
        }else{
            $listing_metas = array();
        }
		$metas = array();
        ...
        foreach ($listing_metas as $k=>$v){
            ...
            $metas[] = (object)array(
                'id'=>$getlistingdata[0]->id,
                'listings_id'=>$getlistingdata[0]->id,
                'meta_key'=>$k,
                'meta_value'=>$v,
            );
        }
		...
		$listing_type = isset($getlistingdata[0]) ? $getlistingdata[0]->listing_type : '';
		...
		if (!empty($posted_data)) {
            ...
            //$imagesname = array_merge($imagesname,$images_already_available_arr);
			$posted_data['images_from'] = serialize($imagesname);
			...
			$this->load->view('frontend/postform/edit_post_form', $data);
		}
	}
	...
	public function edit_ajax_image()
	{
		...
		$get_meta_listing2 = $this->Listings_Model->get_meta_returned_image($id_of_listing);

		//$images_form = unserialize($get_meta_listing2[0]->meta_value);

		echo json_encode($get_meta_listing2);
	}
	public function delete_ajax_image()
	{
		$id_of_listing = $this->input->post('id_of_listing');
		$image_name = $this->input->post('image_name');
		$image_data = $this->input->post('image_data');
		
		if (empty($id_of_listing))
			redirect('sorry');

		$images_form = explode(",", $image_data);
		
		$images = array();
		foreach ($images_form as  $value) {
			if($value != "" && $image_name !== $value){
				$images[] = $value;
			}
		}
	...
	public function add_buying_lead()
	{
        ...
        $jsonManipulation = (new \application\classes\JsonManipulation());
        $get_dynamic_categories = $jsonManipulation->get_categories();

		$data = [
			'countries' => volgo_get_countries(),
			'states' => volgo_get_states_by_country_id(volgo_get_country_id_from_session()),
            'all_new_cats' => $get_dynamic_categories
		];
	...
	public function submit_buying_lead()
	{
		...
		if (!empty($posted_data)) {
			...
			if ($this->form_validation->run() !== FALSE) {
                ...
                $total_premium_users = $this->Users_Model->get_premium_user_by_email();

                $title = $posted_data['input_title'];
                $siteurl = base_url() . volgo_make_slug($title);
                foreach ($total_premium_users as $metas):
                    if ($metas->meta_key == 'pd_email') {
                        $pd_email = $metas->meta_value;
                    }
//				echo '<pre>'; print_r($title);
//				echo '<pre>'; print_r($siteurl); exit;
                    $msg = str_replace('%%title%%', $title, LISTING_ADDED_EMAIL);
                    $msg = str_replace('%%link%%', $siteurl, $msg);
                    $user_data = array('email'=> $pd_email);
                    $emailto = '';
                    $emailfrom = NEWSLETTER_FROM_EMAIL;
                    $subject = 'A New Lead Has Been Added Kindly Visit This Link |' . SITE_NAME;
				...
				if ($is_saved) {
					...
					echo json_encode(['success_msg' => '<p><strong>Note! </strong><br />Your ad is pending for approval.</p>', 'warning_msg' => '<p><strong>Note! </strong><br />Your ad is pending for approval.</p>']);
					exit;
				} else {
					echo json_encode(['validation_error' => '<strong>Error: </strong><br />Unable to save the ad. Kindly retry. If problem persists then kindly contact to administrator. Thank You']);
					exit;
	...
	public function submit_seller_lead()
	{
		...
		if (!empty($posted_data)) {
			...
			if (isset($_FILES['userfile']) && !empty($_FILES['userfile']['name'][0])) {
				...
				// 	echo json_encode(['validation_error' => '<strong>Sorry: </strong> ' . $return_val['errors']]);
				// 	exit;
				// }
			}
	...
	public function handle_fb_login()
	{
		...
		if (empty($user)) {
			...
			if (!empty($user_password)) {
				    ...
				    $siteurl = base_url();
					$msg = str_replace('%%fullname%%', $firstname . ' ' . $lastname, EMAIL_NEW_USER_WELCOME_EMAIL);
                    $msg = str_replace('%%username%%', $fb_user_name, $msg);
                    $msg = str_replace('%%firstname%%', $firstname, $msg);
                    $msg = str_replace('%%lastname%%', $lastname, $msg);
                    $msg = str_replace('%%email%%', $fb_user_email, $msg);
                    $msg = str_replace('%%mobile%%', '', $msg);
                    $msg = str_replace('%%password%%', $user_password, $msg);
	...
	public function user_password_reset()
	{
		...
		if ($this->form_validation->run() !== false) {
			...
			if ($userData && !empty($userData)) {
				...
				$data = array(
					'validation_errors' => '<strong>Sorry</strong><br />Invalid Email',
				);
			}

			// password reset email
                $html = 'email:' . $input_data['user_email'];
                $html = base_url('Users/verify_reset_password/') . volgo_encrypt_message($html);
	...
	public function google_login()
	{
		...
		if (!empty($_GET['error'])) {
			...
			$this->session->set_flashdata('errors', htmlspecialchars($_GET['error']));
			...
			$this->session->set_flashdata('errors', 'Invalid State');
			...
			try {
				...
				// Create User, Send welcome email and send password email.
				if (empty($user)) {
					$is_created = $this->Users_Model->add_user_from_google($email, $name, $state, $code, $session_state, $token_another, $refresh_token, $expires);
					if (!$is_created) {
					...
					$user_password = $this->session->userdata('google_user_password');
					if (!empty($user_password)) {
                        ...
                        $emailfrom = NEWSLETTER_FROM_EMAIL;
                        $subject = 'Your temporary Password |' . SITE_NAME;
                        //volgo_send_email($html,$subject,$emailto,$emailfrom,$data);
                        ...
                        // Welcome Email
						$user_data = array('email' => $email, 'name' => $name);	
                        $emailto = '';
                        $emailfrom = NEWSLETTER_FROM_EMAIL;
                        $subject = 'Welcome |' . SITE_NAME;
                        volgo_send_email($msg,$subject,$emailto,$emailfrom,$user_data);
