...
class MobileTradeshows extends CI_Controller{
	...
	public function index($limit = 10)
	{
		...
		$trade_shows = $this->Tradeshow_Model->get_all_mobile($limit, $page);
        $total_rows = count($trade_shows);
		$this->load->view('frontend-mobile/tradeshows/all-tradeshows', ['tradeshows' => $trade_shows, 'total_rows' => $total_rows]);
	}
    ...
    public function load_more_data()
    {
        ...
        if (isset($tradeshows) && !empty($tradeshows)):
            foreach ($tradeshows as $tradeshow) :
                ...
                $html .= '<div class="tradbox" '. $tradeshow->id. '>';
                $html .= '<div class="trade-content position-relative d-flex">';
                $html .= '<div class="smallpurple"><p>'. date('d M Y', strtotime($tradeshow->date_time));
                $html .= '</p></div>';
                $html .= '<a href='. base_url( 'tradeshow/' . $tradeshow->slug). '>';
                $html .= '<img class="img-fluid imgstyle" src="'. $image . '" alt="trade-show">';
                $html .= '</a>';
                $html .= '</div>';
                ...
                $html .= '</a>';
                $html .= '<p><strong>By</strong> <span class="george">George S.Henry</span></p>';
                $html .= '<p class="tradep">';
	...
	public function show_by_slug($slug = '')
	{
        ...
        $this->load->view('frontend-mobile/tradeshows/tradeshows_detail', $tradeshow);
	}
