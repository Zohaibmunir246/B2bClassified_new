...
class Ajax extends CI_Controller
{
	...
	public function show_by_slug($slug = '')
	{
		...
		$cat_name = volgo_make_slug(strtolower(trim($listing['listing_detail']['info'][0]->category_name)));
		
		$parent_cat_name = $jsonManipulation->get_parent_cat_name_from_cat_id($listing['listing_detail']['info'][0]->category_id);
        ...
        $query = "SELECT * FROM listing_views WHERE ip = '$ip' AND listing_id = ".$detail['info'][0]->listing_id."";
        $result = $this->db->query($query);
        if($result->num_rows() == 0){
            $data['listing_id'] = $detail['info'][0]->listing_id;
            $data['ip'] = $ip;
		...
		if ($cat_name === 'autos' && $json_category_type === 'category') {
			...
			$listing['parent_cat_id'] = $listing['listing_detail']['info'][0]->category_id;
			$listing['popular_searches'] = $this->Listings_Model->popular_searches($country_id,$detail['info'][0]->category_id);
...
// Calculate script execution time
	...
	public function ajax_listing_load(){
        $jsonManipulation = (new \application\classes\JsonManipulation());
