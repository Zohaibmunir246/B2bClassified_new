...
class Merge extends CI_Controller{
    ...
    public function update_slugs($page = 0)
    {
        exit('already updated');
        $is_updated = $this->Merge_Model->update_slug($page);
        if ($is_updated){
            echo '<h1>Slugs updated successfully!</h1>';
            exit;
        }

        volgo_debug($is_updated);
	}

    public function update_currency_code($page = 0)
    {

        $is_updated = $this->Merge_Model->update_currency_code($page);
        if ($is_updated){
            echo '<h1>Currency Code Updated successfully!</h1>';
            exit;
        }
