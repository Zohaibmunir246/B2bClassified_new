...
class Merge_Model extends CI_Model
{
	...
	private function get_old_db_obj()
	{
		return $this->load->database('old_db', true);
    }
    ...
    {
    	$old_db = $this->get_old_db_obj();
		$old_db->select('*');
		$old_db->from($this->old_users_table);
    ...
    public function merge_users()
    {
        ...
        foreach ($this->get_all_old_users() as $user) {
			...
			$new_db_user = $this->db->select('email')->from($this->new_users_table)->where('email', $user->email)->get()->row();

			// check only new users should be addd.
			if ($new_db_user !== null && !empty($new_db_user))
				continue;
    ...
    function get_autos_listings($limit, $offset)
    {
		$old_db = $this->get_old_db_obj();
        $old_db->select('*');
        $old_db->from($this->old_autos_listings_table);

        return $old_db->limit($limit)->offset($offset)->get()->result();
    }

    // get listings from Jan 01, 2019
	public function get_autos_listings_new()
	{
		$old_db = $this->get_old_db_obj();

		$query = $old_db->query("SELECT * FROM listing_autos l WHERE l.timestamp > '2019-01-01 00:00:00'");
		return $query->result();
	}
    ...
    function merge_autos_listings($page)
    {
        ...
        $autos_listings = $this->get_autos_listings_new();
        ...
        foreach ($autos_listings as $auto_listing) {
			...
			$old_db = $this->get_old_db_obj();
            $old_user_data = $old_db->select('email')->from('users')->where('id', $auto_listing->user_id)->get()->row();
            ...
            // Ad images
            $ad_images_arr = $old_db->select('full_image')->from('listing_images')->where('ad_id', intval($auto_listing->id))->where('ad_type_id', $auto_listing->formsubid)->get()->result();
            ...
            // Sub Category name
            $old_subcategory_name = $old_db->select('s.heading')
                ->from('subpages s')
	...
	public function get_all_old_classified_listings_new()
	{
		$old_db = $this->get_old_db_obj();

		$query = $old_db->query("SELECT * FROM listing_classified c WHERE c.timestamp > '2019-01-01 00:00:00'");
		return $query->result();
	}
    ...
    public function listing_classified($page)
    {
        ...
        $classifieds = $this->get_all_old_classified_listings_new();
        $old_db = $this->get_old_db_obj();

        foreach ($classifieds as $classified) {

            // GET user email from user id - from old table
            $old_user_data = $old_db->select('email')->from('users')->where('id', $classified->user_id)->get()->row();
            ...
            // Ad images
            $ad_images_arr = $old_db->select('full_image')->from('listing_images')
	...
	public function get_all_old_jobs_listings_new()
	{
		$old_db = $this->get_old_db_obj();

		$query = $old_db->query("SELECT * FROM {$this->old_job_listings_table} j WHERE j.timestamp > '2019-01-01 00:00:00'");
    ...
    public function listing_jobs($page = 0)
    {
        ...
        $jobs = $this->get_all_old_jobs_listings_new();
        $old_db_obj = $this->get_old_db_obj();
        ...
        foreach ($jobs as $job) {
            ...
            // GET user email from user id - from old table
            $old_user_data = $old_db_obj->select('email')->from('users')->where('id', $job->user_id)->get()->row();
            ...
            // Ad images
            $ad_images_arr = $old_db_obj->select('full_image')->from('listing_images')
            ...
            // Sub Category name
            $old_subcategory_name = $old_db_obj->select('s.heading')
                ->from('subpages s')
	...
	public function get_all_old_jobs_wanted_listings_new()
	{
		$old_db = $this->get_old_db_obj();

		$query = $old_db->query("SELECT * FROM {$this->old_job_wanted_listings_table} j WHERE j.timestamp > '2019-01-01 00:00:00'");
		return $query->result();
	}
    ...
    public function listing_jobs_wanted($page)
    {
        ...
        $jobs_wanted = $this->get_all_old_jobs_wanted_listings_new();
		$old_db_obj = $this->get_old_db_obj();
        ...
        foreach ($jobs_wanted as $job_wanted) {
            ...
            // GET user email from user id - from old table
            $old_user_data = $old_db_obj->select('email')->from('users')->where('id', $job_wanted->user_id)->get()->row();
	...
	public function get_all_old_properties_listings_new()
	{
		$old_db = $this->get_old_db_obj();

		$query = $old_db->query("SELECT * FROM {$this->old_properties_listings_table} p WHERE p.timestamp > '2019-01-01 00:00:00'");
		return $query->result();
	}
    ...
    public function listing_properties_merge($page)
    {
        ...
        $properties = $this->get_all_old_properties_listings_new();
        $old_db_obj = $this->get_old_db_obj();
        ...
        foreach ($properties as $property) {
            ...
            // GET user email from user id - from old table
            $old_user_data = $old_db_obj->select('email')->from('users')->where('id', $property->user_id)->get()->row();
	...
	public function get_all_old_services_listings_new()
	{
		$old_db = $this->get_old_db_obj();

		$query = $old_db->query("SELECT * FROM {$this->old_services_listings_table} s WHERE s.timestamp > '2019-01-01 00:00:00'");
		return $query->result();
	}
    ...
    public function services_listing_merge($page)
    {
        ...
        $services = $this->get_all_old_services_listings_new();
		$old_db_obj = $this->get_old_db_obj();
        ...
        foreach ($services as $service) {
            ...
            // GET user email from user id - from old table
            $old_user_data = $old_db_obj->select('email')->from('users')->where('id', $service->user_id)->get()->row();
    ...
    public function update_slug($page)
    {
        ...
        $query = "SELECT
                ...
                FROM
                    listings l
                GROUP BY l.slug
                HAVING COUNT(l.slug) > 1;";

        $result = $this->db->query($query);
        $result = $result->result_array();

        foreach ($result as $row) {
            $slug_data = $this->db->select('id,slug')->from('listings')
                ->where('slug', $row['slug'])->get()->result();

            $counter = 1;
            foreach ($slug_data as $db_row) {
                $slug = $db_row->slug . '-' . $counter;
                $counter++;
                ...
                $this->db->set($data);
                $this->db->where('id', intval($db_row->id));
                $this->db->update(
                    'listings'
                );

                echo 'Updated Listing ID: ' . $db_row->id . '<br />';
            }
        }
    }

    public function update_currency_code($page)
    {
        $this->print_header_info($page);
        ...
        $listings = $this->db->select('id')->from('listings')->where('country_id', 166)
            ->limit($limit)->offset($offset)->get()->result();

        /*echo ($this->db->last_query()) . '<hr />';
        var_export($listings);
        exit;*/

        /*echo '<pre>' . var_export($listings, true) . '</pre>';
        exit;*/

        foreach ($listings as $listing_row) {
            $meta_info = $this->db->select('*')->from('listings_meta')->where('listings_id', intval($listing_row->id))->where('meta_key', 'currency_code')->get()->row();

            if (empty($meta_info)) {
                $data = array(
                    'meta_key' => 'currency_code',
                    'meta_value' => 'PKR',
                    'listings_id' => $listing_row->id
                );

                $this->db->set($data);
                $this->db->insert('listings_meta');

                echo 'Listing ID: ' . $listing_row->id . '<br />';
            } else {
                if (strtolower($meta_info->meta_value) !== 'pkr') {
                    $data = array(
                        'meta_value' => 'PKR'
                    );

                    $this->db->set($data);
                    $this->db->where('id', intval($meta_info->id));
                    $this->db->update(
                        'listings_meta'
                    );

                    echo 'Listing ID Updated: ' . $listing_row->id . '<br />';
                }
            }
        }
